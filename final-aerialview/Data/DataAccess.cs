    ...
    public class DataAccess
    {
        ...
        public async Task<UserModel> GetUserByUsernameAsync(string userId)
        {
            using (var connection = CreateConnection())
            {
                ...
                var user = await connection.QuerySingleOrDefaultAsync<UserModel>(query, new { UserId = userId });
                return user ?? new UserModel(); // Return a default UserModel if user is null
            }
        }
